...
The parse reasons are `missing_local_role`, `missing_terms`, `recurring_request`,
...
`invalid_payment_target` and `payment_target_not_routable`.

`unsupported_local_role` and `non_actionable_state` are expected filtering of outgoing or
completed records, so they do not emit incoming-rejection warnings.
